...
export default class CirclesGridBeat {
    constructor(cnvs) {
        ...
        const loop = new Tone.Loop(time => {
          if (this.video.readyState === this.video.HAVE_ENOUGH_DATA) {
            ...
            for (let i = 0; i < this.numDataPoints; i++) {
                ...
                const note = this.mapToSample(r);
                const volume = this.mapToVolume(g);
        
                this.samples.triggerAttackRelease(note, "8n", time, volume);
    ...
    mapToSample(value) {
        const sampleMap = ["C4", "D4", "E4", "F4", "G4"];
        const index = Math.floor((value / 255) * sampleMap.length);
